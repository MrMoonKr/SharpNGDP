...
namespace SharpNGDP
{
    public class VersionManager
    {
        ...
        public DownloadFile DownloadFile =>
            _downloadFile
            ?? (_downloadFile = NGDPClient.FileManager.Get(new TACTRequest(CDN, CDNRequestType.Data, GetEKey(BuildConfig.Dictionary["download"].Split(' ')))).GetFile<DownloadFile>());


        private KeyValueFile _cdnConfig;
        public KeyValueFile CDNConfig =>
            _cdnConfig
            ?? (_cdnConfig = NGDPClient.FileManager.Get(new TACTRequest(CDN, CDNRequestType.Config, Version.CDNConfig)).GetFile<KeyValueFile>());
